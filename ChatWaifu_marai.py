...
sound_status=False
qq =2122523627               # 你登录的机器人 QQ 号
verify_key = '06410abcABC'     # 你在 setting.yml 中设置的 verifyKey
    ...
    def reply_to_group(bot: miraicle.Mirai, msg: miraicle.GroupMessage):
        ...
        if msg.at_me() and msg.plain.isspace()==False and sound_status==False:
            if msg.plain.strip() == "重置对话":
                ...
                like = SnowNLP(answer).sentiments
                print(answer)
                print(like)
                bot.send_group_msg(group=msg.group,msg=answer)
                bot.send_group_msg(group=msg.group, msg=str(like))
